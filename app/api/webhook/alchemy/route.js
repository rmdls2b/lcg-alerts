  ...
  try {
    ...
    for (const tx of activities) {
      ...
      await prisma.alert.create({
        data: { addressId: watched.id, txHash: txHash, fromAddr: fromAddress, toAddr: toAddress, amount: String(value), asset: asset, chain: watched.chain },
      })
      ...
      var allEmails = [watched.user.email]
      for (var i = 0; i < watched.recipients.length; i++) {
        allEmails.push(watched.recipients[i].email)
      }
