...
export async function POST(request) {
  try {
    ...
    if (!token || !password || password.length < 8) return NextResponse.json({ error: "Donnees invalides" }, { status: 400 })
    ...
    if (reset.expiresAt < new Date()) return NextResponse.json({ error: "Lien expire" }, { status: 400 })
