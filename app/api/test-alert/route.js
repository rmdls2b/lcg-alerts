...
import { getUserFromRequest, escapeHtml, createRateLimiter, checkEmailLimit } from "../../lib/security"
export const dynamic = 'force-dynamic'
export const fetchCache = 'force-no-store'

const prisma = new PrismaClient()
const resend = new Resend(process.env.RESEND_API_KEY)
const testLimiter = createRateLimiter(3, 60 * 60 * 1000)
...
export async function POST(request) {
  try {
    ...
    if (!testLimiter.check(ip)) {
      return NextResponse.json({ error: "Too many test alerts. Please try again later." }, { status: 429 })
    }
    ...
    if (firstAddress) {
      ...
      ackUrl = APP_URL + "/api/acknowledge?token=" + alert.ackToken
    }
    ...
    const subject = pseudonymLabel ? "[TEST] Emergency signal from " + escapeHtml(pseudonymLabel) + ": immediate action required" : "[TEST] Emergency signal activated: immediate action required"
