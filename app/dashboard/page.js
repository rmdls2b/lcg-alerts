  ...
  if (!auth) return (
    <div className="max-w-[360px] mx-auto mt-32 text-center">
      ...
      <form onSubmit={handleAuth}>
        <input type="password" value={pwd} onChange={function(e) { setPwd(e.target.value) }} placeholder="Admin password"
    ...
    <div className="max-w-[800px] mx-auto px-6 py-8">
      ...
      <div className="grid grid-cols-4 gap-4 mb-10">
        {[
          ...
          { label: "Monitored addresses", value: data.totalAddresses },
          { label: "Real alerts", value: data.totalAlerts },
          { label: "Test alerts", value: data.totalTestAlerts },
        ].map(function(stat) {
          return (
      ...
      <h2 className="text-base text-gray-500 mb-4">Users</h2>
          ...
          <div key={user.id} className="bg-[#111] border border-gray-800 rounded-xl p-4 mb-3 flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-gray-600 mt-1">
                {user._count.addresses} address{user._count.addresses > 1 ? "es" : ""} — registered on {new Date(user.createdAt).toLocaleDateString("en-GB")}
              </div>
            </div>
            <div>
