...
export default function MonEspace() {
  ...
  async function saveInstructions(id) {
    await fetch("/api/addresses/update", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id, instructions: editInstructions[id] }),
    })
    loadData(user.userId)
  }
  ...
  return (
    <div style={{ maxWidth: "700px", margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "32px" }}>
        ...
        <button onClick={logout} style={{ ...btnStyle, backgroundColor: "#222", border: "1px solid #333", color: "#888" }}>
          Deconnexion
        </button>
      </div>
          ...
          <div key={addr.id} style={{ background: "#111", border: "1px solid #222", borderRadius: "8px", padding: "20px", marginBottom: "16px" }}>
            ...
            <div style={{ marginBottom: "16px" }}>
              ...
              {addr.recipients.map(function(r) {
                return (
                  <div key={r.id} style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
                    ...
                    <button onClick={() => removeRecipient(r.id)} style={{ ...btnStyle, backgroundColor: "#331111", color: "#ff6666", padding: "2px 8px", fontSize: "11px" }}>
                      supprimer
                    </button>
                  </div>
                )
              ...
              <div style={{ display: "flex", gap: "8px", marginTop: "8px" }}>
                <input
                  ...
                  onChange={e => setNewEmail({ ...newEmail, [addr.id]: e.target.value })}
                  style={inputStyle}
                />
                <button onClick={() => addRecipient(addr.id)} style={{ ...btnStyle, backgroundColor: "#00d4aa", color: "#000" }}>
                  Ajouter
                </button>
              </div>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                onChange={e => setEditInstructions({ ...editInstructions, [addr.id]: e.target.value })}
              ...
              />
              <button onClick={() => saveInstructions(addr.id)} style={{ ...btnStyle, backgroundColor: "#00d4aa", color: "#000", marginTop: "6px" }}>
                Sauvegarder
              </button>
            </div>
      ...
      <div style={{ marginTop: "40px", borderTop: "1px solid #222", paddingTop: "24px" }}>
        {!confirmDelete ? (
          <button onClick={() => setConfirmDelete(true)} style={{ ...btnStyle, backgroundColor: "#1a0000", border: "1px solid #661111", color: "#ff6666", padding: "10px 20px" }}>
          ...
          <div style={{ background: "#1a0000", border: "1px solid #661111", borderRadius: "8px", padding: "16px" }}>
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              <button onClick={deleteAccount} style={{ ...btnStyle, backgroundColor: "#ff4444", color: "#fff" }}>
                Oui, supprimer
              </button>
              <button onClick={() => setConfirmDelete(false)} style={{ ...btnStyle, backgroundColor: "#333", color: "#ccc" }}>
                Annuler
              </button>
            </div>
          </div>
